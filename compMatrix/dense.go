...
func (A *DenseMatrix) rowSlice(row int) []complex128 {
...
func (A *DenseMatrix) Copy() *DenseMatrix {
	...
	B.cols = A.cols
	B.step = A.cols
	B.elements = make([]complex128, B.rows * B.cols)
...
func Numbers(rows, cols int, num complex128) *DenseMatrix {
	A := Zeros(rows, cols)
...
func Diagonal(d []complex128) *DenseMatrix {
	n := len(d)
	A := Zeros(n, n)
	for i := 0; i < n; i++ {
		A.Set(i, i, d[i])
