...
func (A *SparseMatrix) Get(i, j int) complex128 {
	i = i % A.rows
	if i < 0 {
		i = A.rows - i
	}
...
*/
func (A *SparseMatrix) GetIndex(index int) complex128 {
...
func (A *SparseMatrix) Set(i int, j int, v complex128) {
	i = i % A.rows
	if i < 0 {
		i = A.rows - i
	}
...
*/
func (A *SparseMatrix) SetIndex(index int, v complex128) {
	// v == 0 results in removal of key from underlying map
	if v == 0 {
...
*/
func MakeSparseMatrix(elements map[int]complex128, rows int, cols int) *SparseMatrix {
	A := ZerosSparse(rows, cols)
	A.elements = elements
	return A
}
