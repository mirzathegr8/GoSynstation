...
func (m *DenseMatrix) ScaleRow(r int, f complex128) {
	index := r * m.step
	for j := 0; j < m.cols; j++ {
		m.elements[index] *= f
		index++
...
func (m *DenseMatrix) ScaleAddRow(rd int, rs int, f complex128) {
	indexd := rd * m.step
	indexs := rs * m.step
	for j := 0; j < m.cols; j++ {
...
func (A *DenseMatrix) Det() complex128 {
	B := A.Copy()
	P := B.LUInPlace()
...
func (A *DenseMatrix) TwoNorm() complex128 {
