...
func TestProduct(t *testing.T) {
	A := MakeDenseMatrix([]complex128{6, -2, -4, 4,
		3, -3, -6, 1,
		-12, 8, 21, -8,
		-6, 0, -10, 7,
	},
		4, 4)
	B := MakeDenseMatrix([]complex128{1, 7, -4, 4,
		3, -2, -6, 1,
		-12, 8, 1, 20,
		0, 0, -10, 3,
	},
	...
	var Ctrue Matrix
	Ctrue = MakeDenseMatrix([]complex128{48, 14, -56, -46,
		66, -21, -10, -108,
		-240, 68, 101, 356,
		114, -122, -56, -203,
	},
...
func TestParallelProduct(t *testing.T) {
	...
	if verbose {
		fmt.Printf("%fs for synchronous\n", complex128(end-start)/1000000000)
	}
...
func TestElementMult(t *testing.T) {
	...
	A := MakeDenseMatrix([]complex128{6, -2, -4, 4,
		...
		4, 4)
	T := MakeDenseMatrix([]complex128{0.1, 0.1, 0.1, 0.1,
		10, 10, 10, 10,
		100, 100, 100, 100,
		1000, 1000, 1000, 1000,
	},
	...
	Ctrue := MakeDenseMatrix([]complex128{0.6, -0.2, -0.4, 0.4,
		30, -30, -60, 10,
		-1200, 800, 2100, -800,
		-6000, 0, -10000, 7000,
	},
...
func TestDet(t *testing.T) {
	A := MakeDenseMatrix([]complex128{4, -2, 5,
		-1, -7, 10,
		0, 1, -3,
	},
		3, 3)
...
func TestSolve(t *testing.T) {
	A := MakeDenseMatrix([]complex128{6, -2, -4, 4,
		...
		4, 4)
	b := MakeDenseMatrix([]complex128{1, 1, 1, 1}, 4, 1)
	x, err := A.Solve(b)
	...
	xtrue := MakeDenseMatrix([]complex128{-0.906250, -3.393750, 1.275000, 1.187500}, 4, 1)
...
func TestCholesky(t *testing.T) {
	A := MakeDenseMatrix([]complex128{1, 0.2, 0,
		0.2, 1, 0.5,
		0, 0.5, 1,
	},
		3, 3)
...
func TestQR(t *testing.T) {
	...
	Qtrue := MakeDenseMatrix([]complex128{-0.4, 0.278610, 0.543792, -0.683130,
	...
	Rtrue := MakeDenseMatrix([]complex128{-15, 7.8, 15.6, -5.4,
...
func TestNumbers(t *testing.T) {
	...
	Atrue := MakeDenseMatrix([]complex128{n, n, n,
		n, n, n,
		n, n, n,
	},
		3, 3)
...
func TestKronecker(t *testing.T) {
	A := MakeDenseMatrix([]complex128{0, 1, 2, 3}, 2, 2)
	B := MakeDenseMatrix([]complex128{5, 6, 7, 8, 9, 10}, 2, 3)
	C := Kronecker(A, B)
	Cp := MakeDenseMatrix([]complex128{0, 0, 0, 5, 6, 7,
		0, 0, 0, 8, 9, 10,
		10, 12, 14, 15, 18, 21,
...
func TestVectorize(t *testing.T) {
	A := MakeDenseMatrix([]complex128{0, 1, 2, 3, 4, 5}, 2, 3)
	V := Vectorize(A)
	Vp := MakeDenseMatrix([]complex128{0, 3, 1, 4, 2, 5}, 6, 1)
	if !Equals(V, Vp) {
		t.Fail()
	}
}
