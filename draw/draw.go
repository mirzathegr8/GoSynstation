...
func Draw(syns []synstation.DBS, mobs []synstation.Mob, k int) {
	...
	for i := range syns {
		for e := syns[i].Connec.Front(); e != nil; e = e.Next() {
			...
			data.connec[l].B = *syns[i].R.GetPos()
...
func (d *DataSave) Init(MobileNumber int, ConnectionNumber int) {
	d.mobs = make([]synstation.EmitterS, MobileNumber)
	d.connec = make([]synstation.ConnectionS, ConnectionNumber)
